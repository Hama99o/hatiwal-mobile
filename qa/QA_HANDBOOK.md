...
| **Saved listings** | Yes — per ACCOUNT, so it also crosses devices and reinstalls | Ten flows tap `save-toggle-button`, which is a TOGGLE: tapping it blind unsaves what another flow saved. listing_detail_saves_count lost its "Saved by ..." line that way (gated on savesCount > 0) and listing_detail_save_unsave could never see "No saved items yet" | Read the state before acting — the button's accessibilityLabel flips, "Save listing" vs "Remove from saved" — and assert THIS listing's presence or absence, never a global empty state |

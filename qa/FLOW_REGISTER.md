...
| `dead_end_notice_sold` | FAIL-assert | run-521 | 244 | flow — `listing-unavailable-notice` not visible, but it IS current (ListingUnavailableNotice.tsx:249). Reach/timing or a genuine state mismatch (the notice is SOLD-only per SF-M3 — check the fixture's status is actually `sold` before treating it as UI). Quiet pass, 237s. | [Failed] dead_end_notice_sold (3m 48s) (Assertion is false: id: listing-unavailable-notice is visible) |

...
| `listing_actions_sheet` | FAIL-assert | run-520 | 280 | flow — `Element not found: Id matching regex: browse-tab`, i.e. NO TAB BAR: the flow is standing on a pushed screen. _helpers/pop_to_tab_bar.yaml exists for exactly this and its header notes audit_structure found five flows doing it. Add that helper before the tab tap. | [Failed] listing_actions_sheet (4m 22s) (Element not found: Id matching regex: browse-tab) |

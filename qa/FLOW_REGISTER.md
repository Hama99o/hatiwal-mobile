...
**120 of 251 flows passing** · 129 still need attention

| Status | Count | Meaning |
|---|---:|---|
| PASS | 120 | green, and no backend error underneath |
| SILENT | 1 | **assertions passed while the API errored** — the app told the user nothing |
| FAIL-assert | 68 | an assertion failed — real bug OR a stale selector, triage it |
| FAIL-? | 23 | failed, cause unclear — read the log |
| (rig) | 2 | rig broke mid-run — result meaningless, re-run |
| UNTESTED | 37 | never executed |
...
| `lifecycle_from_chat` | PASS | run-270 | 290 | flow — toast race ("Listing marked as sold"); load-bearing wait, see audit_toasts note |  |
...
| Flow | Status | Last run | Secs | Triage | Notes |
|---|---|---|---:|---|---|
| `held_quantity_refusal` | FAIL-assert | run-271 | 223 |  | [Failed] held_quantity_refusal (3m 28s) (Assertion is false: "Winter Gloves Wholesale Box - 15 Pairs" is visib |
| `listing_actions_sheet` | FAIL-assert | run-266 | 332 |  | [Failed] listing_actions_sheet (5m 12s) (Assertion is false: "Listing published!" is visible) |
| `listing_conversations` | UNTESTED | — |  |  |  |
| `mark_sold_with_buyer` | PASS ⟳stale | run-267 | 196 |  |  |
| `multi_quantity_offplatform_sale` | PASS | run-270 | 224 |  |  |
| `multi_quantity_partial_sale` | FAIL-? | run-271 | 205 | fixed | Found UI-008 (HIGH): typed 3, sold all 15 — pre-filled field appended, clamp silently swallowed it, listing retired. Fixed with selectTextOnFocus + a destructive over-stock hint; same fix applied to the web dialog. Also UI-009 ("15 of 15 left" before any sale). Flow itself needed: explicit seller login (login_seller.yaml lands in the dev-client launcher; login.yaml ignores an EMAIL override when a session exists), scrollUntilVisible on `lifecycle-more-action`, and the review prompt instead of the racing toast. run-020 green, 0 api errors, DB confirms 3 sold / 12 left / still active. |
| `publish_from_owner_detail` | UNTESTED | — |  |  |  |
| `publish_success` | FAIL-assert | run-266 | 459 |  | [Failed] publish_success (7m 19s) (Assertion is false: "Publish Success Test Item" is visible) |
| `reserved_buyer` | UNTESTED | — |  |  |  |
| `sales_screen_correct_quantity` | FAIL-? ⟳stale | run-271 | 362 |  | [Failed] sales_screen_correct_quantity (5m 47s) (No visible element found: id: listing-status-badge) |
| `sales_screen_reviewed_sale_refusal` | FAIL-assert | run-270 | 194 |  | [Failed] sales_screen_reviewed_sale_refusal (2m 59s) (Assertion is false: "Electric Kettle 1.8L Stainless - Bu |
| `sales_screen_void_row` | PASS ⟳stale | run-271 | 370 |  |  |
| `save_draft` | UNTESTED | — |  |  |  |
| `sell_without_reserving` | PASS | run-270 | 303 |  |  |
| `sold_quantity_reconciliation` | FAIL-assert ⟳stale | run-270 | 433 |  | [Failed] sold_quantity_reconciliation (6m 57s) (Assertion is false: "QA SF-M7 Reconcile Batch" is visible) |
| `undo_mark_sold` | PASS | run-271 | 306 |  |  |
| `undo_mark_sold_with_buyer` | PASS ⟳stale | run-269 | 402 |  |  |
...
| `save_listing` | FAIL-? | run-266 | 194 |  | [Failed] save_listing (2m 55s) (No visible element found: "Wool Blanket Handmade King Size") |
...
| `buyer_picker_rtl` | PASS ⟳stale | run-269 | 269 |  |  |
| `categories_hub_rtl` | UNTESTED | — |  |  |  |
| `chat_rtl` | UNTESTED | — |  |  |  |
| `listing_detail_rtl` | FAIL-assert | run-266 | 501 |  | [Failed] listing_detail_rtl (8m 1s) (Assertion is false: id: profile-tab is visible) |
| `my_listings_rtl` | PASS | run-270 | 262 |  |  |
| `profile_quick_actions_rtl` | UNTESTED | — |  |  |  |
| `profile_rtl` | UNTESTED | — |  |  |  |
| `sales_ledger_rtl` | PASS ⟳stale | run-269 | 292 |  |  |
...
| `profile_dark` | FAIL-assert | run-266 | 292 | flow — same toothless restart wait; fixed cb68fa4 | UI-048 OPEN: ended on the Bazaar feed mid-flow, cause not established. Checkpointed. |
...
| `theme_persists_after_navigate` | FAIL-assert ⟳stale | run-265 | 242 | flow — same toothless restart wait; fixed cb68fa4 | UI-048 OPEN: same. Waited on profile-tab, which is visible on every tab. |
...
21/41 passing · 2 open

| Flow | Status | Last run | Secs | Triage | Notes |
|---|---|---|---:|---|---|
| `browse_all_categories` | PASS | run-263 | 207 |  | AxiosError |
| `browse_listings` | PASS | run-266 | 258 |  |  |
...
| `listing_detail` | FAIL-? | run-266 | 204 |  | [Failed] listing_detail (3m 5s) (No visible element found: "Wool Blanket Handmade King Size") |
| `listing_detail_held_units_transparency` | PASS ⟳stale | run-266 | 537 |  |  |
...
| `listing_detail_reserved_contactable` | PASS | run-270 | 152 |  |  |
...
| `search_listings` | FAIL-assert ⟳stale | run-270 | 258 |  | [Failed] search_listings (4m 3s) (Assertion is false: "Phone Case Silicone Clear - Wholesale" is visible) |
...
| `edit_listing_quantity` | FAIL-? ⟳stale | run-266 | 310 |  | [Failed] edit_listing_quantity (4m 51s) (No visible element found: "Edit") |
...
| `lifecycle_reactivate` | PASS ⟳stale | run-266 | 293 | flow | Detail control with the card sheet already open; sheet offers listing-action-activate. |
| `lifecycle_reserve` | PASS ⟳stale | run-252 | 175 |  |  |
| `lifecycle_sold` | FAIL-assert ⟳stale | run-266 | 227 |  | [Failed] lifecycle_sold (3m 27s) (Assertion is false: "Listing marked as sold" is visible) |
| `lifecycle_unpublish` | PASS ⟳stale | run-252 | 175 |  |  |
...
| `rate_buyer_after_sale` | FAIL-assert ⟳stale | run-266 | 222 |  | [Failed] rate_buyer_after_sale (3m 22s) (Element not found: Text matching regex: Mark as Sold) |
...
13/16 passing · 1 open

| Flow | Status | Last run | Secs | Triage | Notes |
|---|---|---|---:|---|---|
| `confirm_email_prompt` | PASS | s2/run-281 | 190 |  |  |
| `guest_browse` | PASS | s2/run-281 | 162 |  |  |
| `guest_offer_redirect` | PASS | s2/run-281 | 245 |  |  |
| `guest_save_redirect` | FAIL-? | s2/run-281 | 192 |  | [Failed] guest_save_redirect (2m 52s) (No visible element found: "Wool Blanket Handmade King Size") |
| `login` | PASS | s2/run-281 | 158 |  |  |
| `login_deep` | PASS | s2/run-281 | 246 |  |  |
| `login_empty_fields` | PASS | s2/run-281 | 143 |  | Request failed with status code Request failed with status code |
| `login_navigate_to_register` | PASS | s2/run-281 | 144 |  |  |

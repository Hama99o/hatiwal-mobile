...
| `meetup_respond` | FAIL-assert | run-521 | 423 | PASS on a quiet box (418s) — and it was deliberately NOT converted to open_listing_by_title (it searches the PARTIAL title "Phone Case" while the helper asserts the exact one). Leaving it alone was right. | PROVEN defect, no reload in its logcat: Accept needs a proposal from the counterpart and nothing seeds one. Now two-party. 1bdaa76 |

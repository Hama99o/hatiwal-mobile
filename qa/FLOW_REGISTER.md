...
**149 of 258 flows passing** · 105 still need attention
...
| FAIL-? | 28 | failed, cause unclear — read the log |
...
| `listing_actions_sheet` | FAIL-assert | run-524 | 358 | TAB-BAR FIX CONFIRMED (run-522 died on its first action with `Element not found: browse-tab`; it now reaches the publish step). STILL OPEN, and NOT fixed by this pass: it fails on the TOAST `"Listing published!"`, which is AFTER the dialog — the dialog assert passed, so the text-tap ambiguity that explains publish_from_owner_detail did not bite here in run-524. Either the toast never came (confirm_dialog answered the wrong control) or it lived its ~3s and died before polling caught it; read commands.json and the screenshot before guessing. The tap was changed to `lifecycle-primary-action` by id anyway — that removes a proven ambiguity class on the same screen, but it is hardening, not the fix for this failure. 0/6 stable fail, so the next verdict is trustworthy. | [Failed] listing_actions_sheet (5m 25s) (Assertion is false: "Listing published!" is visible) |
...
| `publish_from_owner_detail` | FAIL-assert ⟳stale | run-524 | 327 | flow — RE-TRIAGED, my earlier verdict was wrong twice. The screenshot settles it: run-524 ends on the SUCCESS SHEET with the listing ACTIVE — the app published correctly and the flow failed anyway. So this was never a dialog that had not appeared yet, and a longer wait (my 15s extendedWaitUntil) would have failed the same way 15s later. "Publish" names both the form button and the dialog's own confirm button, so the text tap is ambiguous once the dialog is up and Maestro resolves the two matches to one point — scrim (nothing published) or confirm (published before we look); BOTH are in the history. The edit form's button has no testID (ListingForm.tsx:1174), so the flow now answers the dialog only if it is still there and asserts the SUCCESS SHEET instead. Also retracting "publish_success PASSED in the same pass, so publishing works": publish_success is 1/6 and run-524 was its only pass — there is no passing control. | [Failed] publish_from_owner_detail (5m 1s) (Assertion is false: "Publish this listing?" is visible) |
...
| Flow | Status | Last run | Secs | Triage | Notes |
|---|---|---|---:|---|---|
| `browse_rtl_dari` | PASS | s2/run-527 | 336 |  |  |
| `browse_rtl_pashto` | PASS | s2/run-527 | 540 |  |  |
| `buyer_picker_rtl` | (rig) | s2/run-527 | 601 |  |  |
| `categories_hub_rtl` | PASS | s2/run-527 | 306 | flow | 2026-09-02: tapped text "Back" in a flow whose whole purpose is Pashto. The app renders no literal "Back" — BackButton has accessibilityLabel t(common.goBack) (ps شاته ځه) and testID back_button. Now targets the testID. |
| `chat_rtl` | FAIL | s2/run-527 | 560 | flow? | 2026-09-02: expects ps common.send "لیږل", present verbatim. Same language-revert hypothesis as profile_rtl. |
| `listing_detail_rtl` | PASS | s2/run-527 | 570 |  | AxiosError |
| `my_listings_rtl` | PASS | s2/run-527 | 308 |  |  |
| `profile_quick_actions_rtl` | PASS | s2/run-527 | 306 | flow | 2026-09-02 SOLVED, flow bug, b20007e: the language-revert hypothesis was WRONG, and so was the mode-toggle one. Profile.tsx:306 renders this row as `${t('…myListings')} (${count})`, so the text is 'زما اعلانونه (0)' and Maestro's anchored regex could not match the bare label — the three sibling labels carry no suffix, which is why only this row failed. run-379's screenshot shows Profile correctly in seller mode (green tab bar, three tabs, saved-tab gone). Matched as a prefix now. NOT an app bug. |
| `profile_rtl` | PASS | s2/run-527 | 247 | flow? | 2026-09-02: expects fa profile.editProfile "ویرایش پروفایل", which EXISTS verbatim in the locale file — so not a stale selector. Hypothesis: the language-revert bug (fixed 8097ab3) left the app in English after the switch, so no translated string could match. Re-running on a build with that fix. |
| `sales_ledger_rtl` | PASS | s2/run-527 | 401 |  |  |

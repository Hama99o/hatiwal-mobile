...
| `delete_listing` | PASS | run-517 | 196 | flow (IME covers the field below the title) — identical shape and identical fix to create_listing_currency_eur, this campaign's CONFIRMED fix (66c3093: five consecutive failures at sha 198aa2244b02, then run-528 PASS). The flow types the title, which raises the IME over roughly half the window at 360dp, then taps a field below it — and `tapOn` never scrolls. The testID is CURRENT. Added the proven scroll: `scrollUntilVisible` DOWN, timeout 20000, and deliberately NO centerElement (a field near the end of a form cannot be centred). Failed on `listing-form-price-input`. | Toast unwaitable: onDeleted does router.replace, so it fires on a dying screen. Asserts the outcome. |
| `draft_lifecycle` | FAIL-assert | run-517 | 251 | flow (IME covers the field below the title) — identical shape and identical fix to create_listing_currency_eur, this campaign's CONFIRMED fix (66c3093: five consecutive failures at sha 198aa2244b02, then run-528 PASS). The flow types the title, which raises the IME over roughly half the window at 360dp, then taps a field below it — and `tapOn` never scrolls. The testID is CURRENT. Added the proven scroll: `scrollUntilVisible` DOWN, timeout 20000, and deliberately NO centerElement (a field near the end of a form cannot be centred). Failed on `listing-form-price-input`. | Never confirmed the native publish dialog; now via confirm_dialog (android:id/button1). |
| `edit_listing` | PASS | run-517 | 212 | flow (IME covers the field below the title) — identical shape and identical fix to create_listing_currency_eur, this campaign's CONFIRMED fix (66c3093: five consecutive failures at sha 198aa2244b02, then run-528 PASS). The flow types the title, which raises the IME over roughly half the window at 360dp, then taps a field below it — and `tapOn` never scrolls. The testID is CURRENT. Added the proven scroll: `scrollUntilVisible` DOWN, timeout 20000, and deliberately NO centerElement (a field near the end of a form cannot be centred). Failed on `listing-form-price-input`. |  |
| `edit_listing_all_fields` | FAIL-assert | run-517 | 180 | flow (IME covers the field below the title) — identical shape and identical fix to create_listing_currency_eur, this campaign's CONFIRMED fix (66c3093: five consecutive failures at sha 198aa2244b02, then run-528 PASS). The flow types the title, which raises the IME over roughly half the window at 360dp, then taps a field below it — and `tapOn` never scrolls. The testID is CURRENT. Added the proven scroll: `scrollUntilVisible` DOWN, timeout 20000, and deliberately NO centerElement (a field near the end of a form cannot be centred). Failed on `listing-form-description-input`. | Reached for lifecycle-more-action (detail-only) from the list. Card ⋮ route now. |
...
| `edit_listing_remove_photo` | FAIL-assert | run-517 | 202 | flow (arbitrary card, not a fixture) — NOT the off-screen family, and the app was RIGHT. It opened the FIRST `seller-listing-card`, which in run-528 was listing 3317 "Laptop HP": status DRAFT with exactly ONE photo. On a draft the toolbar is [Save Draft | Publish], never the single [Save] this flow taps (`isPublished = existingListing.status !== "draft"`, ListingForm.tsx:451), and removing the only photo empties the gallery — the end-of-flow screenshot is precisely that toolbar over an empty Photos section. Card order is not a fixture: the DB is re-seeded between cycles (the seller's id moved 420 -> 423 -> 426 inside a few hours). Converted to search_my_shop for "School Backpack Bulk Restock - 20 Bags" — ACTIVE with THREE photos (API-checked) and referenced by NO other flow, so mutating its photos cannot disturb another fixture, unlike the Lenovo ThinkPad which the seeder uses as the price-drop target. | Phantom "Remove" confirm (removePhoto has no dialog); also needed a photo to exist. |
| `edit_listing_reorder_photos` | FAIL-assert | run-517 | 233 | flow (arbitrary card, not a fixture) — NOT the off-screen family, and the app was RIGHT. It opened the FIRST `seller-listing-card`, which in run-528 was listing 3317 "Laptop HP": status DRAFT with exactly ONE photo. On a draft the toolbar is [Save Draft | Publish], never the single [Save] this flow taps (`isPublished = existingListing.status !== "draft"`, ListingForm.tsx:451), and removing the only photo empties the gallery — the end-of-flow screenshot is precisely that toolbar over an empty Photos section. Card order is not a fixture: the DB is re-seeded between cycles (the seller's id moved 420 -> 423 -> 426 inside a few hours). Converted to search_my_shop for "School Backpack Bulk Restock - 20 Bags" — ACTIVE with THREE photos (API-checked) and referenced by NO other flow, so mutating its photos cannot disturb another fixture, unlike the Lenovo ThinkPad which the seeder uses as the price-drop target. | HOLLOW: one tap only selects. Now two taps, asserts the hint clears, attaches 2 photos. |
| `expired_listing_badge` | FAIL-assert | run-517 | 169 | flow (below the fold) — the fixture and the app are BOTH right. The API says listing 3279 "Expired Winter Coat Size L" is expired=true (expires_at 2026-09-10), and the `assertVisible: "Expired"` two lines above PASSED, so the flow is on the right listing. "Renew" is the PRIMARY ACTION label on the owner detail's Manage Listing row (useListingLifecycle.ts:475 — the same button that reads "Publish" for a draft and "Mark as Sold" for a live listing), which sits below the gallery, price, analytics and the map. assertVisible means ON SCREEN. Added scrollUntilVisible DOWN. | No expired listing existed at all; expires_at was never seeded. Fixture added. |

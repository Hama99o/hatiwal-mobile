...
| `offer_send_and_accept` | FAIL-assert | run-515 | 244 | flow — ran as #30, BEFORE the helper fix landed. Re-run before triaging; it is in the same family as reserve_after_accept. | 2026-09-05 scroll-to-title lost its race with a 98-listing feed (timeout had already gone 8s->20s). Now uses _helpers/open_listing_by_title.yaml, the same search sequence that keeps browse/listing_detail_held_units_transparency green. |
...
| `quick_replies` | FAIL-assert | run-515 | 213 | flow — asserts the full quick-reply text "Is this still available? Please let me know.". Check that exact string in all 3 locales before treating it as a UI defect. | Exception in thread "Thread-5" java.io.IOException: Command failed (shell,v2,raw:pm list packages --user 0 dev |

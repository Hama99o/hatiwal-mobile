 ...
 * its FIRST load (`isPending`). Before this existed, ListingForm rendered
 * every field with its EMPTY default value during that window — on a
 * mid-range Android device the seller saw a fully blank form for their
 * complete listing, and tapping Publish inside that window fired this
 * card's own "Add Photos, Title, Price, Category, Location to publish this
 * listing" toast for a listing that actually has all of them. This mirrors
 * the real form's section order (photos → title → price → category →
 * description → location) so the loading state reads as "this screen, not
 * ready yet" rather than a generic spinner. `ListingForm` disables the
 * toolbar Save/Publish buttons for the same `isPending` condition.
 *

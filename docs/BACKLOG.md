...
| **Seller sales history screen** | seller trust / records | ⬜ **The one piece of multi-quantity still unbuilt.** `GET /my/transactions` exists and now returns `quantity` per sale, and NO screen renders it — so "who bought how many, across all my listings" is recorded and invisible. A plain stacked list of `SaleBuyerCard`-shaped rows (buyer identity + units + per-unit final price + date + listing), NOT a sales-analytics view: `docs/SPIKE_LISTING_QUANTITY.md` §13.4 flags scope-creep here as the main risk. Tier 2's `sales` array (several buyers per listing on one card) is a separate, later step. |

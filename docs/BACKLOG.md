...
| **T704** | **Fix babel-plugin-jest-hoist crash in 2 test suites** — `SellerListingCard.test.tsx` + `ListingHeader.test.tsx` fail to run (block-body `jest.mock` of `BuyerPickerSheet`); pre-existing since commit f9a2c39 (TASK-TX01), NOT from reviews work. Fix: arrow/object-return factory or `__mocks__` file | ⬜ Not started | _unassigned_ | P2 CI | card #232 · `src/screens/**/__tests__/` |

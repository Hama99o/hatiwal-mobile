...
- **Search bar:** DONE (TASK-Z684) — `src/components/common/SearchBar.tsx` is the one shared implementation; `Browse.tsx`/`BrowseHeader.tsx` (server, caller-owned 400ms debounce, clear X), `Conversations.tsx` (client instant via `filterItems`, clear X) and `SellerProfile.tsx` (client instant, clear X) all use it — no hand-rolled search-input block remains in any of them. `SearchBar` itself is fully controlled with no built-in debounce (an earlier `debounceMs` prop was removed — unused, and its value-sync effect could revert an in-progress keystroke).
...
- **Do:** `<FilterChip active label onPress trailing?>` + `<ChipScrollRow>` (RTL horizontal). `<CategoryChips categories selectedId onSelect includeAll>` composing them + `useCategoryName`.
- **Variants to preserve:** **server-refetch vs client-side filter** stays caller's choice (`SearchBar` only reports `onChangeText`). Browse selection flows into queryKey; SellerProfile/Conversations filter client-side. Fix `"white"` → `colors.primaryForeground` in SellerProfile's chip row (still open). SavedSearchItem is non-toggle w/ delete + `maxWidth`. Keep RTL ordering identical. Status tabs (MyListings) and Conversations chips compose `<FilterChip>` but keep their distinct active-text tokens + leading icons.

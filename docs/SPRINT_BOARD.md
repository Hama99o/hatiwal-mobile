...
- **Blocks**: -
- **BlockedBy**: TASK-TX01
- **ReviewNotes**: CYCLE-4: dev re-reviewed with zero edits and its own verdict was APPROVE — cycle-3 closed every cycle-2 item (sold/bought counter caches with the GREATEST() two-pass backfill, guarded bump_trust_counters!, serializer on :public and :me). Both independent reviews still returned findings. CR: MED transaction.rb:36-48 the safety comment is FALSE — FORM_ATTRIBUTES allows status directly; either remove the comment or add a recompute-counters rake task | MED Profile.tsx:139 borderRightWidth does not flip with the new row-reverse — use ProfileHeader.tsx:126's 1px View divider | MED Profile.tsx:374-396 and :152 hiding a 0 stat leaves an invisible filler — render the 0 or centre a lone stat | LOW wrap sold_with_buyer! + sold! in a transaction (listings_controller.rb:108-116); BuyerPickerSheet.tsx:214-217 skip strands a reserved Transaction. DR: MAJOR Profile.tsx:107-159 only reserve fillers when stats.length>=2 | MED UserProfile.tsx:382-393 the Sold tab needs emptyIcon and emptyAction | MED extend TransactionStatsBadge with variant='pill' rather than forking it | LOW fontSize 12 → text-xs, ChevronRight in RTL, TabPill minHeight 44.
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: CYCLE-4: dev closed all 7 cycle-3 comments (PriceTag md, dropped the isError gate, String() my-listing keys + setQueryData priming, fixed discard guard). CODE REVIEW IS OWED — it errored. Design review returned: MED ListingForm.tsx:661-667 BackHandler is Android-only, so iOS swipe-back can still silently discard the form — add gestureEnabled:false to listing/new + listing/edit/[id] in app/(main)/_layout.tsx:31-32, or usePreventRemove | MED PublishSuccessSheet.tsx has no success haptic — useEffect triggerHaptic('success', reduceMotion), see BACKLOG.md:334 | MED MyListingDetail.tsx:202-217 re-add isError for COPY ONLY (EmptyState title/icon), never for gating — an offline seller is currently told the listing does not exist | LOW PublishSuccessSheet.tsx:104 colors.overlay → colors.darkScrim; :191 inline fontSize 14/600 → 'text-sm font-semibold'; :284 thumb 48 → 64px plus a StatusBadge by the title | INFO FirstMessageSheet.tsx:149-160 has the same title/price gap — worth a follow-up card.
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: CYCLE-4: the dev agent ran the code and design review this card had never received — verified on headless Chromium at 390/768/844/1000/1023/1024px in light and dark, en and ps, confirmed no collision with OwnerListingBar or UnavailableActions, and fixed a real breakpoint bug; committed in 56ec7b0. The two INDEPENDENT reviewers were both spawned and both errored on the session limit, so an independent review is still owed — the dev reviewing its own work does not close this card. Re-review only; do NOT rebuild.
...
- **Acceptance**: On a viewport ≤1023px, scrolling past the inline 'Message seller' CTA reveals a fixed bottom bar showing the listing price plus the Message-seller and Save controls; scrolling back up hides it again; no bar at `lg` and above. The bar is absent for the listing's own seller and for `reserved`/`sold` l…
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: CYCLE-4: dev DONE — rails reworked onto a real GET /listings/:id/similar, seller rail capped at RAIL_SIZE+1 and deduped via a Set, both rails moved into one Suspense-wrapped Server Component (src/components/listing/cross-sell-rails.tsx), committed in 56ec7b0. BOTH REVIEWS ARE OWED — both reviewers were spawned and both errored on the session limit. Re-review only; do NOT rebuild. The cycle-3 design findings this pass was meant to close (no viewAllHref on the Similar rail, no loading state, over-fetch, no boundary between stacked rails, listing-card.tsx:190 hardcoded sizes, missing accessible name) all need re-checking against the new component.
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: CYCLE-4: dev fixed all 8 cycle-3 items (shared conversationPreviewText(), refreshLoadedPages() to stop page truncation, end-reached gating, partial-results empty state, stable-id testIDs). CODE REVIEW IS OWED — it errored. Design review returned a BLOCKING regression: UniversalList.tsx:409 `if (visibleItems.length < perPage*currentPage) return;` compares a POST-filter count against a PRE-filter budget, so ANY filter permanently kills infinite scroll (Conversations.tsx:165-170 filters read/unread, and the partialResults hint at :376-381 becomes a dead end) — drop the heuristic and de-burst with a fetchingMoreRef (a ref, not the stale isFetchingMore) plus the existing currentPage>=totalPages check; UniversalList.test.tsx:586-623 currently locks the bug in and must be fixed too | also: both chip rows are still paddingVertical:7 and need 44pt | chat.search.partialResults says "messages" on a conversations list | conversationPreviewText prints an unlocalized "Offer: 75000 AFN" | SearchBar's 44pt clear box sits inside a 16x16 Animated.View so it is not hit-testable on Android.
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: CYCLE-4 ABORTED: the dev agent died on the session limit before returning, for the THIRD cycle running — no reviewer was ever spawned. This card has never been reviewed since it was first built in cycle 1 (inline seller lifecycle actions on /my-listings via src/components/account/listing-actions.tsx — publish/reserve/sold/renew/unpublish plus useListingLifecycle, committed in c8ffaee). It is being starved rather than blocked: consider giving it a dedicated cycle or splitting it smaller. Needs a dev pass to verify what is on disk, then BOTH reviews.
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: CYCLE-4: dev re-verified only, no edits — the 3 late cycle-3 fixes are present and correct (seedHistory once-per-context flag, authoritative load() at use-search-history.ts:86-107, SearchBox out of the collapsible panel). Both reviews returned. CR: MED search-box.tsx:146-153 plus search-history-panel.tsx:65 have invalid combobox wiring (the wrapper is not the popup, and the popup is role=group) — drop role=combobox/aria-expanded/aria-controls | MED search-box.tsx:65-72 applyTerm() sets focused=false and drops DOM focus | MED e2e/search-history.spec.ts:65-94 is cold-start flaky (9 of 14 failed cold) and :48 headerInput().first() always hits the desktop copy | LOW MOBILE_TO_WEB_MIGRATION.md:61 contradicts the gate at search-box.tsx:63. DR: MED browse-client.tsx:614 the sidebar field needs `hidden lg:block` (duplicate search between 375-1023px) | MED drop `focused` for panelVariant='inline' | MED search-history-panel.tsx:109-110 a scroll row inside a 250px column should wrap at lg+ | LOW hover-tint scoping, p-1 -m-1 focus ring, roving Arrow/Escape nav, saved-searches pill parity, border-border.
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: CYCLE-4: dev DONE — both remaining MED design follow-ups had already landed (AwayBanner inside HideForOwner, OwnerListingBar under the price row), and the dev ran the owed code review and fixed its finding (3 inline ownership copies). CODE REVIEW: APPROVED — the first approval of the run. Design review returned CHANGES_REQUESTED: MED the owner-flash while auth-provider.tsx is 'loading' — ownership is client-side only, so the owner briefly sees buyer-only blocks during the /api/auth/session probe | MED owner-listing-bar.tsx:101-108 View Conversations should use the free conversations_count (listing_serializer.rb:104) | LOW :90 and :104 `truncate` is inert on an inline-flex Button span | LOW :81 and :83 StatusBadge "Active" and ExpiryBadge "expired" contradict each other | LOW :65 aria-label duplicates the <p> | LOW report-button.tsx:140-150 is a raw <button> at ~20px | ALSO from the code review, non-blocking: sellers/[id]/page.tsx:96 AwayBanner has no owner gate, and a draft listing's public URL still notFound()s for its owner.
- **Description**: ## The bug (verified in code) On `hatiwal-web/src/app/[locale]/listings/[id]/page.tsx` an ACTIVE listing renders `StartConversationButton`, `SellerPhoneReveal`, `SaveButton`, `HideListingButton` and `ReportButton` — and every single one returns `null` for the listing's own seller (`start-conversation-button.tsx:57`, `seller-phone-reveal.tsx:28`, `save-button.tsx:59`, `hide-listing-button.tsx:27`, `report-button.tsx:55`). Since `isActive` is true, the `else` notice branch never renders either. So a seller who opens their own listing (from their own share link, Google, a category page, the similar rail) gets an EMPTY action column whose only surviving element is `<SafetyTips>` — buyer 'how to meet safely' guidance shown to the seller about their own item. `listing.detail.ownListingNotice` already exists in all three web locale files and is…
- **Acceptance**: 1) Signed in as the seller, open your own ACTIVE listing at /en/listings/<id>: an owner panel shows the 'this is your listing' notice, the status badge, the expiry pill when <=7 days out, and three working links (Manage → /my-listings/<id>, Edit → /listings/<id>/edit, Conversations → /conversations?…
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: CYCLE-4: dev rewrote all 5 flagged breakages in maestro/chat/reserve_after_accept.yaml plus 2 more found by auditing selectors against the real copy and testIDs. Both reviews returned, and they found the Maestro flow STILL broken: CR 2 BLOCKERS — :61 "Message Seller" should be "Contact Seller" (ListingDetail.tsx:889-901); :66-71 composer-plus-button / composer-action-offer cannot exist yet, so replace :61-71 with a single tapOn "Make an Offer" | HIGH an open design item was dropped: reserveAfterAccept.ts:81-131 still uses the native confirmAlert instead of BuyerPickerSheet | MED ps/chat.json:187 reserve wording is split across 7 keys | MED offer_send_and_accept.yaml has the same defects. DR: BLOCKER the same ps split | BLOCKER move the reserve confirm into BuyerPickerSheet (preselectedBuyer + thumb + PriceTag) | the price inside the alert BUTTON truncates in ps/fa — put it in the body only, with bidi isolates | assert the Reserved badge, not the toast | extract OfferOutcomeBadge/OfferActionsRow from MessageBubble.tsx:681-758 and :885-960.
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: CYCLE-4: dev applied the cycle-3 CR fixes (photos-add-button testID plus Maestro tap, map-pin/photo steps added to two flows, " *" marker on Photos, deduped destructive error block). Both reviews returned and found 3 BLOCKING issues: create_listing_with_condition.yaml:21,23 tap the Labels "Title"/"Price" — now "Title *" — instead of the placeholders; and ListingForm.tsx:527 handlePublishBlockers early-returns when blockers.length===0 but is ALSO the zod onInvalid handler (:562, :584, :609), so genuine validation failures are now SILENT | HIGH :1213-1217 and :1129 setValue for lat/lng/location/categoryId lacks {shouldDirty:true} | MED :661-667 BackHandler sits in a plain useEffect and is always true. DR: HIGH unify all 5 inline errors (:862, :915, :987 are bare text-xs) | HIGH extract components/common/FieldError.tsx — PhotoFieldError is copy-pasted into ListingForm.tsx:1073-1093 | MED delete unreachable PhotosSection.tsx:320-331 and :428 | MED :697-732 Pressables at minHeight 40 → RNR Button at 44pt | LOW photos-add-button id at all 4 sites, a11y labels, haptic.
- **Description**: ## Problem (verified in code)
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: CYCLE-4 ABORTED before any work: the dev agent died on the session limit and no reviewer was spawned, so the backend and screen changes have NOT been built. One artifact does exist — src/screens/chat/__tests__/Conversations.test.tsx.broken, committed in bed7a2d: a test file the dying agent left behind that does NOT compile (it fails inside babel-plugin-jest-hoist at transform time, so it cannot merely be skipped). It was renamed out of Jest's testMatch to keep the suite green, and carries a banner listing everything already ruled out. Start there: fix or discard that file, then build the card.
- **Description**: ## Problem (verified in code)
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: Proposed by a Product Owner in cycle 4; never claimed, nothing built yet.

...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: STATES (saved-searches.tsx): No loading or error state. `const searches = listQ.data ?? []` makes the component render the 'No saved searches' empty branch while listQ.isLoading and on listQ.isError, so authed users see a false empty flash every mount and a silently wrong message on fetch failure. Fix: branch on listQ.isLoading (skeleton rows: bg-muted animate-pulse) and listQ.isError (short error line / reuse existing error pattern) before the searches.length === 0 branch. | TARGETS (saved-searches.tsx lines 176-184): Delete X button is a bare 14px icon (size-3.5) with no padding — hit area ~14px, well below the 40px minimum, versus mobile's 14px hitSlop. Both raw <button> elements (apply line 164-170, delete line 176-184) also lack focus-visible ring styling that the shadcn Button primitive provides, so keyboard focus is invisible. Fix: pad/min-size the delete target (e.g. p-1 or size-8 grid place-items-center) and add rounded-md focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none to both buttons, or use the ghost/icon Button variant. | PARITY minor (saved-searches.tsx lines 157-163 vs mobile SavedSearchItem.tsx 116-117): Mobile emphasizes a search with new matches by drawing a 1.5px primary-colored border around the whole chip; web relies on the badge alone, so the row is a weaker 'new' signal. Recommend conditionally applying border-primary on the <li> when (s.newMatchesCount ?? 0) > 0. | Confirmed OK: Badge uses shared ui/badge default variant with only token overrides (px-1.5 tabular-nums) — library-compliant; all colors are theme tokens (dark mode works); RTL handled via html dir + text-start + gap, no physical margins; ICU plural newMatches keys present in en/ps/fa matching mobile wording; proxy allow-list entry correctly scoped to PUT users/saved_searches/<id>/mark_seen; optimistic zero + invalidate on settle mirrors mobile.

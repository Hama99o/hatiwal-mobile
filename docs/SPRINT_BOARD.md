...
- **Blocks**: -
- **BlockedBy**: TASK-TX01
- **ReviewNotes**: CYCLE-3 (dev DONE — closed every open cycle-2 item; both earlier BLOCKERs had already landed in ce2fe48, and the MAJOR contradiction between the lifetime soldCount trust badge and the Sold showcase tab after a sold listing is removed is now fixed). BOTH REVIEWS ARE OWED — they died in the session limit. Re-review only; do NOT rebuild.
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: CYCLE-3 (both reviews returned): DR BLOCKER PublishSuccessSheet.tsx:196 PriceTag size="sm" (13sp) → size="md" (17sp) so the price outranks the title | CR MyListingDetail.tsx:351 the `isError||!listing` guard destroys the success sheet on a failed background refetch — use `if(!listing)` | CR ListingForm.tsx:446-462 add qc.setQueryData(["my-listing",String(id)],listing) before dismissTo and normalise BOTH screens to string query keys | CR ListingForm.tsx:605-622 the unsaved-changes guard misses the Android hardware back button — add usePreventRemove/BackHandler | CR ListingForm.routing.test.tsx:52-204 is the 4th verbatim copy of the same harness — extract __tests__/helpers/listingFormHarness.tsx | CR routing.test.tsx:217 comment and coordinate fill are wrong in the draft branch | CR missing the edit-then-publish case dismissTo("/(main)/my-listings/42?published=1").
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: CYCLE-3 ABORTED: the dev agent died on the session limit before returning, for the second cycle running. This card has never been reviewed since it was built in cycle 1 — the sticky mobile buyer action bar in src/components/listing/listing-action-bar.tsx (IntersectionObserver-gated, reusing StartConversationButton/SaveButton), plus e2e/listing-action-bar.spec.ts. It needs BOTH a code and a design review, and a check that it does not collide with the owner bar (TASK-WEB-OWN947) or the recovery CTAs (TASK-WEB-SOLDNEXT) on the same page.
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: CYCLE-3 (both reviews returned; seller rail now wrapped in HideForOwner, home page uses the shared ListingRail, 10 listing-detail E2E specs added): CR MUST add getSimilarListings(id) hitting listings/:id/similar instead of re-approximating similarity client-side | CR SHOULD pageSize RAIL_SIZE+1 and dedupe the two rails via a Set of already-shown ids | DR page.tsx:405-410 the Similar rail has no viewAllHref — a dead end and asymmetric with the seller rail | DR both rails block render with no loading state | DR 3x over-fetch (pageSize 12 sliced to 4) | DR no visual boundary between the two stacked rails | DR listing-card.tsx:190 hardcoded sizes no longer match the rail tracks | DR listing-rail.tsx:59-81 section has no accessible name and context-free link text | DR listings hidden via "Not interested" reappear through the anonymous SSR fetch. NOTE the tsconfig/.next-e2e scratch churn the reviewer flagged has already been cleaned up.
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: CYCLE-3 (both reviews returned; the cycle-2 fixes were confirmed already landed in ce2fe48): CR filterConversations.ts:46 matches a different field than ConversationRow.tsx:86-106 displays — extract a shared conversationPreviewText(item,t) | CR UniversalList.tsx:313 REGRESSION: loaded pages are truncated on every focus — refetch pages 1..currentPage or add preservePagesOnRefresh | CR UniversalList.tsx:325 end-reached fires in a burst — gate on visibleItems.length>=perPage*currentPage | CR Conversations.tsx:331 shows a false "No matches" — append chat.search.partialResults | CR Conversations.tsx:295 key/testID/animation keyed off the wrong value, use item.id | DR UniversalList.tsx:472-506 add keyboardShouldPersistTaps="handled" + keyboardDismissMode="on-drag" | DR SearchBar needs autoCapitalize/autoCorrect/spellCheck off and real padding:14 on the clear button (not hitSlop) | DR Conversations.tsx:418-456 targets under 44pt — paddingVertical 12/10.
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: CYCLE-3 ABORTED: the dev agent died on the session limit before returning, for the second cycle running. This card has now never been reviewed at all since it was first built in cycle 1, when it landed inline seller lifecycle actions on /my-listings via src/components/account/listing-actions.tsx (publish/reserve/sold/renew/unpublish plus useListingLifecycle), committed in c8ffaee. It needs a dev pass to verify what is on disk, then BOTH a code and a design review. Any partial work is committed in 41b133c.
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: CYCLE-3 (dev DONE — re-verified all 11 merged fix-list items after the outage, found 3 still open and fixed them: seedHistory now uses a once-per-context flag, use-search-history got a proper load(), and the SearchBox moved out of the 375px filter panel). BOTH REVIEWS ARE OWED — they died in the session limit. Re-review only; do NOT rebuild.
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: CYCLE-3 ABORTED: the dev agent died on the session limit before returning. The cycle-2 state still stands — the fix-up is committed in 9713ba8 and its DESIGN REVIEW PASSED (the only approval in the run so far); the code review is still owed. Remaining design follow-ups: MED listings/[id]/page.tsx:266 AwayBanner is not owner-gated, so an away seller reads "Seller is away" about themselves — gate it like mobile ListingDetail.tsx:706 | MED page.tsx:276-286 the owner panel sits ~2 screens down while ListingActionBar self-suppresses for owners — render OwnerListingBar right after the price/meta block | LOW page.tsx:143-145 pb-28 reserves 112px for a bar never rendered for the owner — move the spacer into ListingActionBar | LOW owner-listing-bar.tsx:65,73 aria-label duplicates the visible <p> — use aria-labelledby | LOW owner-listing-bar.tsx:81 a draft shows no badge or publish affordance.
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: CYCLE-3 (dev DONE — closed the genuinely open categories-hub items; the aborted cycle-2 agent had already landed with_counts, hub error/empty states, de-emphasised empty cards, shared category-badge chips and the Rails roll-up). BOTH REVIEWS ARE OWED — they died in the session limit. Re-review only; do NOT rebuild.
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: CYCLE-3 (dev PARTIAL — mobile side closed out, both reviews returned): CR MUST maestro/chat/reserve_after_accept.yaml is broken in five places — :69 asserts buyerPicker.soldTitle, use "Who's buying this item?"; :63 "Accepted" never renders, use "Offer accepted"; :37 must tap composer-plus-button first because K487 moved the offer entry point; :46/61/67-71 unanchored selectors need `.*`; :51 testID chat-tab does not exist, tapOn "Chats" | CR acceptance #9 still unmet: the hatiwal-web mirror is in this card's build list but is outside the mobile boundary — SPLIT IT OUT as its own type:web card rather than leaving this one open forever | CR ps/chat.json listingActions.reserve still collides with common.save | CR Conversation.tsx:602 currency precedence untested — hoist resolveReserveCurrency | DR HIGH reserveAfterAccept.ts:81-114 uses native Alert.alert, which breaks dark mode and RTL and is not RNR — move it into BuyerPickerSheet via preselectedBuyer/presetFinalPrice, and put {{price}} in the success copy.
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: CYCLE-3 (dev DONE — the aborted cycle-2 commit ce2fe48 already held a complete implementation: pure getPublishBlockers, scrollToBlocker, photosError, localized publishBlocked toast. CODE REVIEW IS STILL OWED — it died in the session limit — so this card needs a code review, not a rebuild). Design review returned: create_listing_publish_blocked.yaml:68 "Add Photos" is ambiguous while the blocking toast is showing — extendedWaitUntil notVisible first, or add a testID | create_listing_publish_direct.yaml:36-42 and create_listing_with_condition.yaml:26-28 need the 6-step map-picker block plus a photo and a pin | PhotosSection.tsx:198-201 Photos is the only publish-required field with no " *" marker — use the RNR Label | PhotosSection.tsx:229-236 (duplicated at 391-398) bare text-xs error — text-sm plus AlertCircle in an isRtl row | ListingForm.tsx:1238-1254 topBtn minHeight 40 → 44 | PhotosSection.tsx:281-295 the with-photos error wrapper is unreachable — delete it | PhotosSection.tsx:547-585 badges do not mirror — use start/end | ListingForm.publish.test.tsx:201 needs jest.setTimeout(20000).
- **Description**: ## Problem (verified in code)
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: CYCLE-3 ABORTED: the dev agent died on the session limit before returning, for the second cycle running. Real partial work IS on disk and committed (9713ba8 then 41b133c) — the chat bubble meta row, message-bubble and conversation-thread changes, plus an e2e/chat-receipts.spec.ts. Read what is already there and finish it; do NOT restart from scratch. Still owed both reviews.
...
- **Blocks**: -
- **BlockedBy**: -
- **ReviewNotes**: Proposed in cycle 3; its dev agent died on the session limit before starting, so NOTHING has been built yet — this card is untouched and ready to claim.
